...
def detect_facial_features(image_np):
    ...
    if len(faces) > 0:
        ...
        # Simple mouth curve analysis (lower face region)
        # Check if region is valid before slicing
        if h * 0.9 > y + h or w * 0.8 > x + w or h * 0.6 < y or w * 0.2 < x:
            # Fallback for small or poorly detected faces
            mouth_region = roi_gray[int(h*0.6):int(h*0.9), int(w*0.2):int(w*0.8)]
        else:
             mouth_region = roi_gray[int(h*0.6):int(h*0.9), int(w*0.2):int(w*0.8)]

        if mouth_region.size > 0:
            # Calculate horizontal gradients to detect mouth curve (measures edge intensity)
            sobelx = cv2.Sobel(mouth_region, cv2.CV_64F, 1, 0, ksize=3)
            # Normalize the curve measurement - sad/neutral faces usually have less horizontal texture
            mouth_curve = np.mean(np.abs(sobelx))
            features['mouth_curve'] = min(1.0, mouth_curve / 50.0) 
...
def smart_feature_predict(image_np, debug_container):
    """
    Revised prediction based on facial features and expression analysis.
    The weights have been adjusted to remove the bias towards "HAPPY" and
    increase the penalties for "NOT_HAPPY" features.
    """
    with debug_container:
        try:
            # 1. Analyze features and characteristics
            ...
            # --- Applying Feature Factors (Weights adjusted for balance) ---
            
            # Factor 1: Smile detection (Strongest positive indicator, but reduced weight)
            if facial_features['smile_detected']:
                # SIGNIFICANT CHANGE: Reduced positive weight from +0.4 to +0.25
                base_prediction += 0.25
                st.info("✅ Smile detected (+0.25 HAPPY)")
            else:
                # SIGNIFICANT CHANGE: Increased negative penalty for no smile from -0.05 to -0.2
                # This ensures the score drops strongly into the 'NOT_HAPPY' range if a smile is not found.
                base_prediction -= 0.2
                st.info("❌ No smile detected (-0.2 NOT_HAPPY bias)")
            
            # Factor 2: Mouth curve analysis
            if facial_features['mouth_curve'] > 0.7:
                base_prediction += 0.2 
                st.info(f"⬆️ Strong mouth curve ({facial_features['mouth_curve']:.2f}) (+0.2 HAPPY)")
            elif facial_features['mouth_curve'] < 0.3:
                # SIGNIFICANT CHANGE: Increased negative penalty for low curve (straight/sad mouth)
                base_prediction -= 0.3
                st.info(f"⬇️ Weak/low mouth curve ({facial_features['mouth_curve']:.2f}) (-0.3 NOT_HAPPY bias)")
            
            # Factor 3: Eye detection (Clarity of face, less tied to emotion)
            if facial_features['eyes_detected'] >= 2:
                base_prediction += 0.05
                st.info("👀 Both eyes clear (+0.05 HAPPY)")
            else:
                base_prediction -= 0.05 
                st.warning("❓ Face/eyes unclear (-0.05 HAPPY)")
                
            # Factor 4: Face ratio (wider is sometimes happier, but less critical)
            if facial_features['face_ratio'] > 1.2:
                base_prediction += 0.05
                st.info(f"📏 Wide face ratio ({facial_features['face_ratio']:.2f}) (+0.05 HAPPY)")
            elif facial_features['face_ratio'] < 0.9:
                base_prediction -= 0.05
                st.info(f"📐 Narrow face ratio ({facial_features['face_ratio']:.2f}) (-0.05 HAPPY)")
            ...
            if characteristics['texture_complexity'] > 0.3:
                ...
                base_prediction -= 0.1
                st.info(f"🌫️ Low texture/edges ({characteristics['texture_complexity']:.2f}) (-0.1 NOT_HAPPY bias)")
            
            # Factor 6: Brightness (darker images often correlate with non-happy mood/environment)
            if characteristics['brightness'] < 80:
                base_prediction -= 0.1 # Increased penalty slightly to reflect poor lighting
                st.warning(f"💡 Dark image ({characteristics['brightness']:.1f}) (-0.1 NOT_HAPPY bias)")
            elif characteristics['brightness'] > 180:
                base_prediction += 0.05
                st.info(f"🌟 Bright image ({characteristics['brightness']:.1f}) (+0.05 HAPPY)")
                
            # Add small randomness for variation
            random_adj = random.uniform(-0.02, 0.02) # Reduced randomness effect
            base_prediction += random_adj
            st.write(f"✨ Random adjustment: {random_adj:.3f}")
            
            # Ensure final prediction is within bounds [0.01, 0.99]
            prediction_score = max(0.01, min(0.99, base_prediction))
            st.write(f"📝 Final raw score for HAPPY: {prediction_score:.3f}")
            
            # 3. Determine Final Result
            if prediction_score > 0.5:
                mood_label = HAPPY_LABEL
                # Confidence is how far from 0.5 we are towards 1.0
                confidence = (prediction_score - 0.5) * 2
            else:
                mood_label = NOT_HAPPY_LABEL
                # Confidence is how far from 0.5 we are towards 0.0
                confidence = (0.5 - prediction_score) * 2
            
            # Normalize confidence back to 0.5-1.0 range for display
        ...
        if st.button("🚀 Process Batch", use_container_width=True):
            ...
            for i, image_file in enumerate(uploaded_batch):
                ...
                try:
                    ...
                    # Make prediction
                    # The smart_feature_predict function now returns a 0.0-1.0 confidence score
                    mood_label, confidence = smart_feature_predict(image_np, temp_log_container)
...
with tab3:
    ...
    st.warning("""
    **CRITICAL UPDATE APPLIED (Weight Rebalancing):** The original logic was heavily biased towards 'HAPPY' due to the overwhelming positive weight of the smile detection factor. 
    The system has been re-tuned with significantly stronger negative penalties to ensure it correctly identifies neutral or sad expressions when a smile is absent.
    """)
    ...
    **Prediction Mechanism:**
    The final 'HAPPY' score starts at 0.5 (neutral) and is calculated by adding/subtracting weight based on feature presence. The weights are now calibrated for a more neutral, feature-based assessment.
    """)
